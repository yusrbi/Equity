...
public class FinalExperiments_old {
	...
	static Experiment[] experiments = {new Experiment(SETTINGS_ID.DEFAULT, EXPERIMENT_ID.full_src, "New_exp_full_src"), 
			new Experiment(SETTINGS_ID.DEFAULT, EXPERIMENT_ID.full_dest, "New_exp_full_dest"),
			new Experiment(SETTINGS_ID.DEFAULT, EXPERIMENT_ID.part_src, "New_exp_part_src"),
			new Experiment(SETTINGS_ID.DEFAULT, EXPERIMENT_ID.part_dest_cohen, "New_exp_part_dest_cohen"),
			new Experiment(SETTINGS_ID.DEFAULT, EXPERIMENT_ID.part_dest_orig, "New_exp_part_dest_orig"),};
...
//	};
	public static void main(String arg[]) throws IOException {
		...
		for (String file : files) {
			...
			try {
				run_experiemnt(dir,file, full, source);
				slogger_.info("Done!!!!");
	...
	private static void run_experiemnt(File dir, String file, boolean full, boolean source)
	...
	//	document.getTable(0).writeTableWithAnnotationsToDB();
		...
		for (Experiment experiemnt : experiments) {
			try {
				...
				document.setExperiement_id("3_"+experiemnt.id+ 
						(full? "_full":"_part") + (source? "_src":"_dest"));
				Node.reset_counter();
				Edge.reset_counter();
