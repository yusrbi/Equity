...
public class HyperparamsSelector_part {
	...
	public static void main(String arg[]) throws IOException {
		...
		HyperparamGenerator.getHyperparamGenerator().generate(600);
//		HyperparamGenerator.getHyperparamGenerator()
//				.writeHyperparamsToFile("/GW/D5data/yibrahim/misc/jars/hyperparams_600_redo.lst");
	...
	private static void run_experiemnt(File dir, Document_ document, boolean full, boolean source, boolean initDB)
		...
		for (double[] hyperparams : HyperparamGenerator.getHyperparamGenerator().getHyperparams()) {
			try {
				...
				document.setExperiement_id(String.format("3107_orig_Hyperparams_Estimation_%d_%s_%s",id,  (full? "full":"part") , (source? "src":"dest")));
...
//					document.getTable(0).writeResultstoDB();
					//TODO before un-commenting delete the results :D 
					//
					//document.deleteResults();
					document.setExperiement_id(String.format("3107_cohen_Hyperparams_Estimation_%d_%s_%s",id,  (full? "full":"part") , (source? "src":"dest")));
					Solver.doRandomWalk_fromCandidates_cohen(document, 0);
					slogger_.info("Cohen Done with RWR from Candidates on partial graph");
				}
				document.writeResultstoDB();
				document.getTable(0).writeResultstoDB();
				id++;
